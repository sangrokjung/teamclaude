...
**Model-scoped weekly windows** (`anthropic-ratelimit-unified-7d_<label>-*`, currently `7d_oi` — the separate Opus-tier limit shown as "Fable" in Claude's usage UI) are parsed into `quota.modelWeekly`, displayed in the TUI/status, and applied only when the incoming request maps to that model tier. An Opus-exhausted account is skipped for Opus requests but remains available to Sonnet/Haiku; a live model-quota 429 also follows this model-local path rather than globally throttling the account.

...
test('exact OAuth rejection never replays the POST onto a mixed-pool API-key account', async () => {
...
test('Codex POST 401 and 429 responses remain single-dispatch across mixed credentials', async t => {
  for (const status of [401, 429]) {
    for (const firstType of ['oauth', 'api_key']) {
      await t.test(`${status}-${firstType}`, async () => {
        const secondType = firstType === 'oauth' ? 'api_key' : 'oauth';
        let hits = 0;
        const upstream = http.createServer(async (req, res) => {
          await readJson(req);
          hits += 1;
...
test('unsafe Anthropic POST preserves a generic 429 without replaying another account', async () => {
  let hits = 0;
  const upstream = http.createServer(async (req, res) => {
    await readJson(req);
    hits += 1;
...
test('Codex request logs contain metadata only and omit bodies, identity, and unlisted headers', async () => {
  ...
  const upstream = http.createServer(async (req, res) => {
    ...
    res.end(JSON.stringify({ detail: `ordinary validation failure for ${responseEmail}` }));
  });
  const manager = new AccountManager([{

...
2. 분류된 계정·모델 조합은 bounded TTL 동안 선택에서 제외합니다. 다른 모델과 계정의
   상태·quota·subscription은 변경하지 않습니다. TTL 만료 뒤 자동 재탐색하며 해당 모델의
   2xx를 관찰하면 즉시 격리를 해제합니다.
...
4. 같은 모델의 모든 eligible OAuth 계정이 이전의 독립적인 exact 거절로 격리된 상태에서
   ...
   content-length를 dispatch 전에 갱신합니다. chain이 없거나 소진되면 기존 429 dead-end를
   반환합니다.
...
10. Codex request의 첫 dispatch에서 정한 OAuth/API-key 인증 유형은 그 request 전체에
    ...
    사용합니다. 일반 unsafe 429는 재전송하지 않고, 명시적 account/model quota exhaustion만
    같은 인증 유형 안에서 기존 bounded failover/fallback 계약을 유지합니다.
...
12. 명시적 account/model quota exhaustion이 아닌 일반 unsafe `POST` 429는 provider와 무관하게
    다른 account로 재전송하지 않고 원 status/body/header를 반환합니다. replay-safe method만 기존
    bounded failover 계약을 사용할 수 있습니다.
...
- 일반 Anthropic/Codex unsafe POST 429는 upstream hit가 1회이고 원 429를 반환합니다.
...
- gzip Codex SSE 뒤 같은 keep-alive agent의 status 요청이 timeout 없이 200이며, SSE encoded
  body와 encoding/length가 upstream과 일치합니다.
...
- fresh 독립 적대 검토 2개 lane에서 CRITICAL/HIGH가 0건입니다.
...
- unsafe request의 generic 429와 ambiguous 5xx는 provider와 무관하게 내부 재전송하지 않습니다.

...
정확히 식별된 subscription-disabled OAuth 403을 account-scoped 접근 차단 상태로 격리하고, 같은 request를 다른 사용 가능한 account로 즉시 재전송합니다. 일반 permission 403은 account 상태를 바꾸거나 재전송하지 않습니다. 조직 접근이 정상화되면 독립 자동 재검증으로 계정을 rotation에 복귀시킵니다.
...
8. `subscriptionRecheckIntervalMs` 자동 재검증은 quota `warmupIntervalMs`와 독립적으로 동작해야 하며, 정상 probe 2xx에서 persistent flag를 제거합니다.
...
- `warmupIntervalMs: 0`에서도 격리 이후 `subscriptionRecheckIntervalMs`가 지나면 정상 probe로 자동 복귀합니다.
...
- 자동 probe는 exact `subscription-disabled` 격리만 복구합니다. `auth-revoked` 등 다른 인증 오류는 기존 계약대로 re-import/login 또는 명시적 운영 조치로만 복구합니다.
...
3. 모든 account가 error면 각 조직의 Claude Code subscription access를 관리자에게 확인하고 자동 재검증 또는 TUI `R`을 사용합니다. credential 자체가 바뀐 경우에만 re-import/login합니다.

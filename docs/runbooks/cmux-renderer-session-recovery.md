...
> 메인 cmux와 별도 worker tmux 사이의 SSH 단절, hidden local tmux, duplicate
> client, exact native UUID 복구는
> [메인 cmux ↔ worker Codex 세션 복구 및 재발 방지](./main-worker-codex-session-recovery.md)를
> 함께 적용한다.
